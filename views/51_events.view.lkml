...
#     timeframes: [time, date, hour, time_of_day, hour_of_day, week, day_of_week_index, day_of_week]
  ...
  dimension: viewed_product_id {
    ...
    type: number
    sql: CASE WHEN upper(${event_type}) = 'PRODUCT' THEN
  ...
  dimension: os {
    # This column is missing in thelook_ecommerce public dataset. So, populated using browser temporarily until os column is created in the public dataset.
    label: "Operating System"
    view_label: "Visitors"
    sql: CASE WHEN ${browser} in ('IE', 'Chrome') THEN 'Windows'
              WHEN ${browser} in ('Safari') THEN 'Macintosh'
              WHEN ${browser} in ( 'Firefox') THEN 'Linux'
              WHEN ${browser} in ( 'Other') THEN 'Linux'
              ELSE 'Android' END ;;
  }
